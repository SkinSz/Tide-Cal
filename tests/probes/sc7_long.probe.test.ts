...
test("SC6 quarantine during active sync: invalid record mid-stream, valid flow continues", async () => {
  ...
  // Pkg6: fixed the reviewer probe's dynamic import (was ../qa-tmp/probes/,
  // pre-existing broken path that made the whole probe file error instead of
  // running its SC6/SC7 assertions). Now: the promoted helper module.
  const { createSyncEngine, makeEntityMutator } = await import(
    "./sync_probe_helpers.ts"
  );
