...
- **Status:** RESOLVED-CORE (2026-08-27) — durable lifecycle state + display landed; full resolution UI (manual Retry/Delete with confirmation, DC-15 §3.5 pattern) and retention/cleanup rule remain future items. Commit pending. **Integration follow-up (one line):** `src/sync/sync_engine.ts` `revalidateQuarantine` should call `reconcileQuarantineResolutions(deps.db)` (exported from `src/persistence/database.ts`) after a pass with `revalidated > 0`; until hooked, resolution is derivable on demand and fully tested in `tests/td005_quarantine_lifecycle.test.ts`. Schema v3 adds `quarantine.resolved_at_hlc` / `resolved_reason` (NULL = active; rows never deleted, no delete path added).
